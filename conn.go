...
func readLoop(c WriteCloser, wg *sync.WaitGroup) {
	var (
		...
		handlerCh        chan MessageHandler  //包含了消息和消息处理函数
		msg              Message  //message类型，包含了消息本身和序列化方法，使用框架时注册
		err              error
	)
	...
	for {
		...
		case <-sDone: // server closed
			...
			return
		default:
			msg, err = codec.Decode(rawConn) //从netconn中解码得到消息
			if err != nil {
			...
			setHeartBeatFunc(time.Now().UnixNano())  //更新这个conn的心跳时间
			handler := GetHandlerFunc(msg.MessageNumber())  //根据消息的类型，拿到消息处理函数
			if handler == nil {
				if onMessage != nil {
			...
			handlerCh <- MessageHandler{msg, handler}  //放入handlerch交给后面处理
		}
	}
}
...
func writeLoop(c WriteCloser, wg *sync.WaitGroup) {
	...
	OuterFor:
		for {
			select {
			case pkt = <-sendCh:  //接到退出信号后，还有从发送通道接到序列化好的内容就发送，没有就退出
				if pkt != nil {
...
func handleLoop(c WriteCloser, wg *sync.WaitGroup) {
	...
	for {
		...
		case <-sDone: // server closed
			holmes.Debugln("receiving cancel signal from server")
			return
		case msgHandler := <-handlerCh:  //从readloop中接到消息
			msg, handler := msgHandler.message, msgHandler.handler
			if handler != nil {
				if askForWorker {
					err = WorkerPoolInstance().Put(netID, func() {
						handler(NewContextWithNetID(NewContextWithMessage(ctx, msg), netID), c) //把请求的标识netid和msg都放入context中，连同此conn一起放在handler中，由worker调用
					})
					if err != nil {
						holmes.Errorln(err)
					}
